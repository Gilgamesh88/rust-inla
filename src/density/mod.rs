...
impl Density {
    ...
    pub fn quantile(&self, q: f64) -> f64 {
        ...
        for _ in 0..50 {
            ...
            if cdf_mid < q { lo = mid; } else { hi = mid; }
        }
        (lo + hi) / 2.0
    }
}
...
fn interpolate_log_density(z: f64, nodes: &[f64], log_density: &[f64]) -> f64 {
    ...
    if z <= nodes[0]            { return log_density[0]; }
    if z >= *nodes.last().unwrap() { return *log_density.last().unwrap(); }
