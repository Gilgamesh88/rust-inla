...
use inla_core::models::{IidModel, Rw1Model, Ar1Model, QFunc};
use inla_core::likelihood::{GaussianLikelihood, PoissonLikelihood, GammaLikelihood, ZipLikelihood, TweedieLikelihood, LogLikelihood};
use inla_core::inference::{InlaParams, InlaModel, InlaEngine};
...
fn rust_inla_run(
    ...
    let n_constr = n_constr_arg as usize;
    let extr_constr_slice = if n_constr > 0 {
        extr_constr_arg.as_real_slice()
    } else {
        None
    };
