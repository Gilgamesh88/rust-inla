...
impl Problem {
    ...
    /// Implementa el General Fixed Effects Solver via Matrix Schur Complement
    /// Reemplaza beta0 con un vector K-dimensional beta.
    pub fn find_mode_with_fixed_effects(
        ...
        if n_fixed == 0 || x_mat.is_none() {
            let (x, ld, di) = self.find_mode_with_inverse(qfunc, likelihood, y, x_idx, theta, x_init, max_iter, tol)?;
    ...
    pub fn find_mode_with_logdet(&mut self, qfunc: &dyn QFunc, likelihood: &dyn crate::likelihood::LogLikelihood, y: &[f64], x_idx: Option<&[usize]>, theta: &[f64], max_iter: usize, tol: f64) -> Result<(Vec<f64>, f64), InlaError> {
        let (x, ld, _) = self.find_mode_with_inverse(qfunc, likelihood, y, x_idx, theta, &[], max_iter, tol)?;
        Ok((x, ld))
    }

    pub fn find_mode(&mut self, qfunc: &dyn QFunc, likelihood: &dyn crate::likelihood::LogLikelihood, y: &[f64], x_idx: Option<&[usize]>, theta: &[f64], max_iter: usize, tol: f64) -> Result<Vec<f64>, InlaError> {
