...
evaluate_case <- function(case) {
    ...
    if (!rusty_perf$ok || !rinla_perf$ok || !inherits(rinla_perf$res, "inla")) {
        return(list(
            ...
            hyper_mean = NULL,
            hyper_sd = NULL,
            linear_predictor_mean = NULL,
            linear_predictor_sd = NULL,
            summary = data.frame(
                case_id = case$id,
    ...
    list(
        ...
        hyper_sd = hyper_sd,
        linear_predictor_mean = linear_predictor_mean,
...
print_case_report <- function(result) {
    ...
    cat(sprintf(
        "Fixed sd max abs diff   : %s\n",
        format(round(result$fixed_sd$max_abs, 6), nsmall = 6)
    ))
    cat(sprintf(
    ...
    cat(sprintf(
        "Hyper mean max abs diff : %s\n",
        format(round(result$hyper_mean$max_abs, 6), nsmall = 6)
    ))
    cat(sprintf(
        "Hyper sd max abs diff   : %s\n",
        format(round(result$hyper_sd$max_abs, 6), nsmall = 6)
    ))
    cat(sprintf(
        "Linear mean max abs diff: %s\n",
        format(round(result$linear_predictor_mean$max_abs, 6), nsmall = 6)
    ))
    cat(sprintf(
        "Linear sd max abs diff  : %s\n",
        format(round(result$linear_predictor_sd$max_abs, 6), nsmall = 6)
    ))
